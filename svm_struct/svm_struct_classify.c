...
char testfile[200];
char modelfile[200];
char wordFeaturefile[200];
char predicateFeaturefile[200];
char structfile[200];
char predictionsfile[200];

void read_input_parameters(int, char **, char *, char *, char *, char *, char *, char *, 
			   STRUCT_LEARN_PARM *, long*, long *);

//void read_input_parameters(int, char **, char *, char *, char *,char *, char *, long *, long *,
...
int main (int argc, char* argv[])
{
  ...
  read_input_parameters(argc,argv,testfile,wordFeaturefile,predicateFeaturefile,modelfile,structfile,
			predictionsfile,&sparm,&verbosity,&struct_verbosity);
  ...
  */
  ...
  testsample=read_struct_examples(testfile,wordFeaturefile,predicateFeaturefile,structfile,&sparm);
  if(struct_verbosity>=1) {
    printf("done.\n"); fflush(stdout);
  }
  ...
  int label_size = testsample.examples[0].x.label_size;
  //int **confusion;
  //confusion=(int**)malloc(sizeof(int*)*label_size);
  //for (i=0; i<label_size; i++) {
  //  confusion[i] = (int*)malloc(sizeof(int)*label_size);
  //  for (j=0; j<label_size; j++) {confusion[i][j]=0;}
  //}
  ...
  for(i=0;i<testsample.n;i++) {
    ...
    //  confusion[testsample.examples[i].y.labels[j]][y[j]]++;
    //}

    write_label(predfl,y,testsample.examples[i].y,testsample.examples[i].x);
  ...
  //write_confusion(predfl,confusion,label_size);
  fclose(predfl);
...
void read_input_parameters(int argc,char *argv[],char *testfile, char *wordFeaturefile, 
			   char *predicateFeaturefile, char *modelfile, char *structfile, 
			   ...
			   long *verbosity,long *struct_verbosity)
{
  ...
  i++;
  strcpy (wordFeaturefile, argv[i]);
  i++;
  strcpy (predicateFeaturefile, argv[i]);
  i++;
  strcpy (structfile, argv[i]);
...
void print_help(void)
{
  ...
  copyright_notice();
  printf("   usage: svm_struct_classify [options] example_file word_feature_file predicate_feature_file structure_file model_file output_file\n\n");
  printf("	   structure_file -> file to store relation link");
  printf("	   word_feature_file -> file to store word specific features");
  printf("	   predicate_feature_file -> file to store predicate specific features");
