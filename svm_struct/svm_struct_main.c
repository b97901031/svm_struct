...
{  
  ...
  read_input_parameters(argc,argv,trainfile,wordFeaturefile,predicateFeaturefile,modelfile,structfile,
                        &verbosity,&struct_verbosity,&struct_parm,&learn_parm,
			&kernel_parm,&alg_type);

  //printf("end read input parameters\n");

  if(struct_verbosity>=1) {
    printf("Reading training examples..."); fflush(stdout);
  }
  /* read the training examples */
  sample=read_struct_examples(trainfile,wordFeaturefile,predicateFeaturefile,structfile,&struct_parm);
  if(struct_verbosity>=1) {
    printf("done\n"); fflush(stdout);
  }
...
void read_input_parameters(int argc,char *argv[],char *trainfile,
			   char *wordFeaturefile, char * predicateFeaturefile, 
                           char *modelfile, char *structfile,
			   ...
			   int *alg_type)
{
  ...
  struct_parm->joint=1;
  struct_parm->multiple_predicates=1;
  struct_parm->decode=1;
  ...
  for(i=1;(i<argc) && ((argv[i])[0] == '-');i++) {
    switch ((argv[i])[1]) 
      ...
      case 'x': i++; struct_parm->multiple_predicates=atol(argv[i]); break;
      case 'z': i++; struct_parm->decode=atol(argv[i]); break;
      case 'j': i++; struct_parm->joint=atol(argv[i]); break;
...
void print_help()
{
  ...
  copyright_notice();
  printf("   usage: svm_struct_learn [options] example_file word_feature_file predicate_feature_file, structure_file  model_file\n\n");
  printf("Arguments:\n");
  ...
  printf("         -x int   -> multiple predicates.0/1\n");
  printf("         -z int   -> decode method: 0=beam, 1=gibbs\n");
  printf("         -j int   -> joint: 0=gold standard, 1=class only, 1=classification+identification\n");
