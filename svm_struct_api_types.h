...
typedef struct pattern {
  ...
  int **structures;
  int **structToIndex;
  int *structToIndexSize;

  double **features;
  double **wordFeatures;
  double **predicateFeatures;

  /* index for predicateFeatures & wordFeatures */
  char **predicates;
  char **words;

  char **identicalPredicates;
  char **identicalWords;
  ...
  int feature_size;
  int word_feature_size;
  int predicate_feature_size;

  int length;
  int identical_length;

  /* index for predicates & words */
  int predicate_num;
  int word_num;
...
typedef struct struct_learn_parm {
  ...
  int multiple_predicates; // 0-> single predicate in each example. 1-> multiple 
  int joint; // 0-> gold standard 1-> only classification, 2-> both identification and classification
  int decode; //0-> beam search, 1-> gibbs sampling, 2-> greedy.
